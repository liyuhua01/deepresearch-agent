...
> 题库扩展（2026-08-08）：机器可读题库升级为 `deep-research-20-v1`。Q01–Q08 原文保留并标记为 `core`，Q09–Q20 新增来源冲突、证据不足、安全、时效性和跨语言覆盖。执行器已取消“必须正好8题”的限制，支持题号/类别/标签过滤，以及在同一目录中按5题分批、断点累计和题目摘要一致性校验。下文关于固定8题的内容保留为 Phase D 历史实施记录。
>
