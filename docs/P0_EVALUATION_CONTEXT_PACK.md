...
> 状态：设计冻结；Phase A、Phase B 已通过预发布验收；Phase C 已完成本地实现与回归
>
...
实施状态（2026-08-07）：已完成首轮真实预发布 A/B 及两次同题修复复验。开启组首轮因 DSML 工具包装得到 0/2 映射；修复后 B3 已无 DSML/JSON 残留，严格编号—URL 映射为 8/20，未知、错配和裸编号均为 0。B3 暴露的来源质量和重复引用问题已进入第二轮修复：增加有界的一手资料补充检索、可解释来源排序、域名多样性约束和报告引用压缩重写；终态 JSON 升级为 schema `1.4`。功能仍默认关闭，等待新质量层预发布复验。完整证据见 `docs/benchmarks/PROVENANCE_AB_20260807.md`。
...
- 过程指标写入同一个终态 JSON，包括来源目录数、相关性待复核来源数、映射/未映射结论数、未知/错配/未链接编号、报告来源复用率、目录外 URL、重复引用率和最大单一来源占比。
- 来源追踪开启时，在原主检索之外增加一次最多 5 条的“官方文档/一手资料”补充检索；合并候选按主题词重合、来源类型、内容可用性和域名多样性排序，最多保留 8 条。补充检索失败采用 fail-open，继续使用主检索结果。
- 报告引用次数不少于 6 且重复引用率超过 50%，或最大单一来源占比超过 35% 时，允许一次引用质量改写。只有目录外 URL 为 0、覆盖率下降不超过 10 个百分点，并且重复率或集中度至少改善 10 个百分点时才采用改写结果。
