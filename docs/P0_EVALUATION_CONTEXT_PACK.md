...
> 状态：设计冻结；Phase A、Phase B 已通过预发布验收；Phase C 已完成主链路接入和单题预发布复验；Phase D 尚未实现
>
...
实施状态（2026-08-07）：已完成首轮真实预发布 A/B 及 B2–B6 同题修复复验。开启组首轮因 DSML 工具包装得到 0/2 映射；B3 修复了工具包装和编号—URL 校验。B5 捕获到搜索后端忽略官方域名限定、返回滑雪场页面的真实失败；B6 增加域名校验、受限多引擎恢复和离题来源剔除后，得到 3/3 Python 官方文档、3/3 URL HTTP 200、5/5 过程结论映射，且未知、错配、裸编号和待复核来源均为 0。重复引用率仍为 76.92%，单题不足以支持默认上线；功能继续默认关闭，下一门槛是 Phase D 固定 8 题执行器。完整证据见 `docs/benchmarks/PROVENANCE_AB_20260807.md`。
