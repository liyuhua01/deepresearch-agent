...
实施状态（2026-08-07）：已完成首轮真实预发布 A/B。开启组的最终结论引用覆盖率为 80%，但过程映射因总结器只返回 DSML 工具调用包装而得到 0/2，且 43 次引用仅对应 5 个唯一 URL，因此保持默认关闭。修复轮已增加工具包装清洗与总结恢复请求、来源编号—URL 配对校验、重复引用/来源集中度指标和保守相关性标记；统一终态 JSON 升级为 schema `1.3`。完整首轮证据见 `docs/benchmarks/PROVENANCE_AB_20260807.md`。
