...
实施状态（2026-08-07）：已完成首轮真实预发布 A/B 及两次同题修复复验。开启组首轮因 DSML 工具包装得到 0/2 映射；修复后 B3 已无 DSML/JSON 残留，严格编号—URL 映射为 8/20，未知、错配和裸编号均为 0。当前主要瓶颈已转为搜索来源质量和重复引用：B3 的 5 个来源中 3 个被标记为相关性待复核，重复引用率为 84.38%。因此继续保持默认关闭。统一终态 JSON 已升级为 schema `1.3`，完整证据见 `docs/benchmarks/PROVENANCE_AB_20260807.md`。
