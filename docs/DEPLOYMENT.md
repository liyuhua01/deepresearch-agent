...
来源追踪 schema `1.4` 还会记录编号—URL 错配、裸编号、重复引用率、最大单一来源占比、权威来源数、需要相关性复核的来源数，以及引用质量改写是否尝试/采用。开启后，每个子任务会增加一次有界的一手资料补充检索；补充检索失败时继续使用主检索结果。相关性规则只负责排序和风险标记，不直接删除全部弱匹配证据；总结器和报告器不得让被标记来源单独支撑核心结论。
