...
来源追踪 schema `1.5` 还会记录编号—URL 错配、裸编号、报告 URL 可追溯率、引用来源主题相关率、来源有效率、重复引用率、最大单一来源占比、权威来源数、需要相关性复核的来源数，以及引用质量改写是否尝试/采用。最终报告中的来源编号会确定性转换为 `[1](URL)` 形式的可点击数字引用；无法映射的内部结论编号显示为“待验证”。开启后，每个子任务会增加一次有界的一手资料补充检索；补充检索失败时继续使用主检索结果。相关性规则只负责排序、初筛和风险标记，不等同于语义支持率；这些来源指标不设置统一高阈值。
